    ...
    public class SalesOrderHandler
    {
        ...
        public Entity ReplicateQuoteVehicleAccessories(Entity salesOrderEntity)
        {
            ...
            foreach (Entity quoteAccessory in quoteAccessoryCollection.Entities)
            {
                ...
                CreateVehicleAccessories(salesOrderEntity, itemDescription, itemId);
            }
            ...
            if (quoteId == Guid.Empty)
            {
                ...
                if (VehicleAccessoriesItems != null && VehicleAccessoriesItems.Entities.Count > 0)
                {
                    ...
                    foreach (var VehicleAccessories in VehicleAccessoriesItems.Entities)
                    {
                        ...
                        CreateVehicleAccessories(salesOrderEntity, ItemNumber, ItemDescription);
                    }
                }
            }
            return salesOrderEntity;
        ...
        private void CreateVehicleAccessories(Entity salesOrderEntity, String itemNumber, EntityReference itemId)
        {
         ...
         * Primary Entity: Order
         */
        public Money FreeInsurance(Entity salesOrderEntity)
        {
            _tracingService.Trace("Started FreeInsurance method ...");

            var isfree = salesOrderEntity.GetAttributeValue<Boolean>("gsc_free");

            if (salesOrderEntity.Contains("gsc_totalpremium"))
            {
                Decimal insurance = Decimal.Zero;

                if (isfree == false)
                {
                    _tracingService.Trace("Insurance is Free");

                    insurance = salesOrderEntity.Contains("gsc_totalpremium")
                        ? salesOrderEntity.GetAttributeValue<Money>("gsc_totalpremium").Value
                        : Decimal.Zero;
                }
                ...
                _tracingService.Trace("Ended FreeInsurance method ...");
        ...
        * This formula applies when there is discount applied for Amount Financed.*/
        ...
        public Entity SetNetPriceAmount(Entity salesOrderEntity, String message)
        {
            ...
            Decimal totalPrice = unitPrice + colorPrice + freightAndHandling;
            ...
            Decimal netamountfinanced = Decimal.Zero;
            ...
            if (paymentMode == 100000001)
            {
                ...
                netamountfinanced = ComputeNetAmountFinanced(salesOrderEntity);
                salesOrderEntity["gsc_netamountfinanced"] = new Money(netamountfinanced);
                salesOrderEntity["gsc_totalamountfinanced"] = new Money(netamountfinanced);
            }
            else if (paymentMode == 100000000 || paymentMode == 100000002 || paymentMode == 100000003)
            {
                salesOrderEntity["gsc_amountfinanced"] = new Money(0);
                salesOrderEntity["gsc_netamountfinanced"] = new Money(0);
        ...
        public Decimal ComputeNetPrice(Entity quoteEntity)
        {
            ...
            var lessdiscount = quoteEntity.GetAttributeValue<Money>("gsc_discount") != null
                ? quoteEntity.GetAttributeValue<Money>("gsc_discount").Value
                : 0;
            ...
            var totalprice = unitprice + colorprice + freightPrice;
       ...
       */
        ...
        public Decimal ComputeAmountFinanced(Entity salesOrderEntity)
        {
            ...
            //var netdp = salesOrderEntity.Contains("gsc_netdownpayment")
            //    ? salesOrderEntity.GetAttributeValue<Money>("gsc_netdownpayment").Value
            //    : 0;
            var downpayment = salesOrderEntity.Contains("gsc_downpaymentamount")
                ? salesOrderEntity.GetAttributeValue<Money>("gsc_downpaymentamount").Value
            ...
            //var amountfinanced = netprice - netdp;
        ...
        private Decimal ComputeNetAmountFinanced(Entity salesOrderEntity)
        {
            ...
            var lessdiscount = salesOrderEntity.Contains("gsc_discountamountfinanced")
                ? salesOrderEntity.GetAttributeValue<Money>("gsc_discountamountfinanced").Value
                : 0;
            var netamountfinanced = amountfinanced - lessdiscount;
        ...
        public void ReplicateQuoteCabChassis(Entity salesOrderEntity)
        {
            ...
            EntityCollection quoteCabChassisCollection = CommonHandler.RetrieveRecordsByOneValue("gsc_sls_quotecabchassis", "gsc_quoteid", quoteId, _organizationService,
                null, OrderType.Ascending, new[] { "gsc_financing", "gsc_itemnumber", "gsc_vehiclecabchassisid", "gsc_quotecabchassispn" });
        ...
        private Entity PopulatePaymentSummary(Entity salesOrderEntity)
        {
            ...
            EntityCollection quoteRecords = CommonHandler.RetrieveRecordsByOneValue("quote", "quoteid", quoteId, _organizationService, null, OrderType.Ascending,
                ...
                "gsc_totalamountdue", "gsc_totalcashoutlay", "gsc_amountfinanced", "gsc_netmonthlyamortization" });
